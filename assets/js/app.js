...
const updateCountries = (display = countries) => {
    let result = '';
    display.forEach(ele => {
        result += ` 
            <div class="col-lg-2">
...
const onKeyUp = (eve) => {
    ...
    const filteredCountries = countries.filter(country => {
        return country.name.toLowerCase().includes(searchWords) || (country.capital && country.capital.toLowerCase().includes(searchWords)) || country.languages.some(lang => lang.toLowerCase().includes(searchWords))
          
    })
    updateCountries(filteredCountries)
    feedback.innerHTML = `<em><strong>${filteredCountries.length}</strong> of ${countries.length} countries found</em>`
}
